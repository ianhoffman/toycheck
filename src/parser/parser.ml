...
and parse_at_exp tokens =
  ...
  | token :: _ -> raise (ParseError (token, "INT, TRUE, FALSE, VAR, LET, LPAREN"))
  | _ -> raise (UnexpectedEndOfStream "INT, TRUE, FALSE, VAR, LET, LPAREN")
  
and parse_app_exp tokens =
  let rec recur tokens absyn =
    if next_is_at_exp tokens
    then
      let (inner, rest) = parse_at_exp tokens
      in recur rest (Term.App (absyn, inner))
    else (absyn, tokens)
  in let (at_exp, rest) = parse_at_exp tokens
  in recur rest at_exp

and parse_inf_exp tokens =
    let rec recur tokens absyn =
      match tokens with
      | Token.Add :: tokens ->
        let (inner, rest) = parse_inf_exp tokens
        in recur rest (Term.App(Term.App(Term.Var("+"), absyn), inner))
      | Token.Sub :: tokens ->
        let (inner, rest) = parse_inf_exp tokens
        in recur rest (Term.App(Term.App(Term.Var("-"), absyn), inner))
      | _ -> (absyn, tokens)
    in let (atExp, rest) = parse_app_exp tokens
    in recur rest atExp
...
and parse_exp tokens =
  ...
  | token :: _ -> raise (ParseError (token, "INT, TRUE, FALSE, VAR, LET, LPAREN, IF, FN"))
  | _ -> raise (UnexpectedEndOfStream "INT, TRUE, FALSE, VAR, LET, LPAREN, IF, FN")

let parse_tokens tokens =
  try (
    match parse_exp tokens with
    | (absyn, []) -> absyn
    | (_, token :: _) -> raise (ParseError (token, "INT, TRUE, FALSE, VAR, IF, FN, LET"))
  ) with
    ParseError (t, expected) ->
      let () = Printf.printf "Backtrace: %s\n" (Printexc.get_backtrace ()) in
      let () =  Printf.printf "Got %s, expected %s\n" (Token.show_token t) expected
      in raise (ParseError (t, expected))
